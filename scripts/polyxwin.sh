...
if [ ! $exit_code -eq 0 ]; then
	...
	wm_class=$(xprop -id $(xdotool getactivewindow) WM_CLASS | awk 'NF {print $NF}' | sed 's/"/ /g' | awk '{$1=$1};1')
	wm_name=$(xprop -id $(xdotool getactivewindow) WM_NAME | cut -d '=' -f 2 | awk -F\" '{ print $2 }' | awk '{$1=$1};1')
